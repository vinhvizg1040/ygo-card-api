function filterCards(data, params) {
    ...
    return data.filter(card => {
        ...
        if (params.fname && !card.name.toLowerCase().includes(params.fname.toLowerCase())) return false;
        ...
        // Filter by level
        if (params.level && card.level !== parseInt(params.level, 10)) return false;

        // Filter by race
        if (params.race && !params.race.includes(card.race)) return false;

        // Filter by attribute
        if (params.attribute && !params.attribute.includes(card.attribute)) return false;

        // Filter by link
        if (params.link && card.link !== parseInt(params.link, 10)) return false;

        // Filter by linkmarker
        if (params.linkmarker && !params.linkmarker.every(marker => card.linkmarkers.includes(marker))) return false;

        // Filter by scale
        if (params.scale && card.scale !== parseInt(params.scale, 10)) return false;

        // Filter by cardset
        if (params.cardset && !card.card_sets.some(set => set.set_name === params.cardset)) return false;
        ...
        // Filter by staple
        if (params.staple && card.staple !== (params.staple === 'true')) return false;

        // Filter by has_effect
        if (params.has_effect && card.has_effect !== (params.has_effect === 'true')) return false;
        ...
        if ((params.startdate || params.enddate) && params.dateregion) {
            ...
            if ((startDate && releaseDate < startDate) || (endDate && releaseDate > endDate)) {
                return false;
            }
        }
